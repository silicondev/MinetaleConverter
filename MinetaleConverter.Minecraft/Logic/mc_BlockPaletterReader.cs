...
{
    public class mc_BlockPaletterReader : INbtConvertReader
    {
        public object? Convert(JObject reader)
        {
            ...
            for (int i = 0; i < jPalette.Count; i++)
            {
                ...
                var jProps = item.Value<JToken>("Properties");
                if (jProps != null)
                {
                    foreach (JProperty child in jProps.Children())
                        properties.Add(child.Name, child.Value.ToString());
                }
            ...
            return new mc_ResourcePalette(palette.ToList(), data, 4);
        }
    }
}
