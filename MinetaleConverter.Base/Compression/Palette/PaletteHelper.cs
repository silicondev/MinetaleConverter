...
namespace MinetaleConverter.Base.Compression.Palette
{
    ...
    {
        public static int[] GetIndices<T>(Palette<T> palette, Func<T, bool> predicate, int minBits = 1)
        {
            //if (palette.Count() == 0)
            //    return [];
            ...
            //return list.ToArray();
            return palette.Decompress().ToNumberedDictionary().Where(x => predicate(x.Value)).Select(x => x.Key).ToArray();
